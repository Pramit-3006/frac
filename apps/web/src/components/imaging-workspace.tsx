import { Crosshair, Maximize2, Ruler, ScanLine, SlidersHorizontal } from "lucide-react";
import { fractureTypes } from "@/lib/mock-data";
import { Panel, PanelHeader } from "./ui";

function ViewerPane({ title, overlay }: { title: string; overlay?: "heatmap" | "mask" | "boxes" }) {
  return (
    <div className="relative min-h-72 overflow-hidden rounded-md border border-white/10 bg-[#06090c] xray-grid">
      <div className="absolute left-3 top-3 z-10 rounded bg-black/50 px-2 py-1 text-xs text-slate-200">{title}</div>
      <div className="absolute inset-8 rounded-[48%] border border-slate-500/30 bg-gradient-to-b from-slate-300/20 via-slate-200/10 to-transparent blur-[1px]" />
      <div className="absolute left-[46%] top-[18%] h-[66%] w-8 rotate-[-8deg] rounded-full border border-slate-200/35 bg-slate-100/15" />
      <div className="absolute left-[53%] top-[39%] h-28 w-[3px] rotate-[28deg] bg-white/70 shadow-[0_0_18px_rgba(255,255,255,0.75)]" />
      {overlay === "heatmap" && <div className="heatmap absolute inset-0 mix-blend-screen" />}
      {overlay === "mask" && <div className="absolute left-[48%] top-[40%] h-32 w-14 rotate-[12deg] rounded-full border-2 border-[#7bdcb5] bg-[#2b7a78]/25" />}
      {overlay === "boxes" && <div className="absolute left-[44%] top-[38%] h-36 w-28 rounded border-2 border-signal" />}
      <div className="absolute bottom-3 right-3 flex gap-2">
        {[Ruler, Crosshair, Maximize2].map((Icon) => (
          <button key={Icon.name} className="rounded border border-white/10 bg-black/40 p-2 text-slate-200" title={Icon.name}>
            <Icon size={16} />
          </button>
        ))}
      </div>
    </div>
  );
}

export function ImagingWorkspace() {
  return (
    <Panel className="col-span-8">
      <PanelHeader
        title="AI Analysis Workspace"
        action={
          <div className="flex items-center gap-2">
            <button className="rounded-md border border-white/10 px-3 py-1.5 text-xs text-slate-200">DICOM</button>
            <button className="rounded-md bg-clinical px-3 py-1.5 text-xs font-medium text-white">Run Ensemble</button>
          </div>
        }
      />
      <div className="grid gap-4 p-4 xl:grid-cols-2">
        <ViewerPane title="Original X-ray" />
        <ViewerPane title="Enhanced + FCET/CLAHE" overlay="boxes" />
        <ViewerPane title="Segmentation Mask" overlay="mask" />
        <ViewerPane title="XAI Heatmap" overlay="heatmap" />
      </div>
      <div className="grid gap-4 border-t border-white/10 p-4 lg:grid-cols-4">
        <div className="rounded-md border border-white/10 bg-[#121b23] p-3">
          <p className="text-xs text-slate-400">Fracture type</p>
          <p className="mt-2 text-lg font-semibold text-white">Oblique, distal radius</p>
        </div>
        <div className="rounded-md border border-white/10 bg-[#121b23] p-3">
          <p className="text-xs text-slate-400">Severity score</p>
          <p className="mt-2 text-lg font-semibold text-amber">7.8 / 10</p>
        </div>
        <div className="rounded-md border border-white/10 bg-[#121b23] p-3">
          <p className="text-xs text-slate-400">Displacement</p>
          <p className="mt-2 text-lg font-semibold text-white">4.2 mm</p>
        </div>
        <div className="rounded-md border border-white/10 bg-[#121b23] p-3">
          <p className="text-xs text-slate-400">Angular deviation</p>
          <p className="mt-2 text-lg font-semibold text-white">12.6 deg</p>
        </div>
      </div>
      <div className="flex flex-wrap gap-2 border-t border-white/10 p-4">
        <span className="flex items-center gap-2 text-xs text-slate-400"><ScanLine size={14} /> Supported labels</span>
        {fractureTypes.map((type) => (
          <span key={type} className="rounded border border-white/10 px-2 py-1 text-xs text-slate-300">{type}</span>
        ))}
        <button className="ml-auto flex items-center gap-2 rounded-md border border-white/10 px-3 py-1 text-xs text-slate-200">
          <SlidersHorizontal size={14} /> Calibrate
        </button>
      </div>
    </Panel>
  );
}

